...
- 解決するIssueの内容を端的に表現する
- Issue番号、変更の種類、影響度を含める
- 形式：`[種類/影響度] [#Issue番号] 変更内容の要約`
- 例：
  - `[Feature/Breaking] [#123] スクロールバーのAPIを刷新`
  - `[Feature/Minor] [#124] スムーズスクロール機能を追加`
  - `[Bug Fix/Patch] [#125] スクロール位置の計算を修正`
...
- 依存モジュールの状態（必須）
  - 変更がない場合：
    - package.jsonとlock fileを確認の上「依存モジュールの変更なし」と明記
    - これによりレビュアーの確認負荷を軽減
  - 変更がある場合は以下を記載：
    - 更新されたモジュール
      - バージョン変更の詳細（必須）
